...
class User(AbstractBaseUser):
    username = models.CharField("사용자 계정", max_length=20, unique=True) # 중복되지 않도록 unique=True
    email = models.EmailField("이메일 주소", max_length=100)
    password = models.CharField("비밀번호", max_length=128)
    fullname = models.CharField("이름", max_length=20)
    join_date = models.DateTimeField("가입일", auto_now_add=True) # create : auto_now_add=True
    ...
    # 로그인 할 때 id로 사용 할 필드를 username으로 설정
    # 로그인 시 USERNAME_FIELD에 설정 된 필드와 password가 사용된다.
    USERNAME_FIELD = 'username'
    # USERNAME_FIELD = 'email'

    # super user를 생성할 때 입력받은 필드 지정. email 번거로워서 잘 안씀.
    # 사용하지 않더라도 선언은 되어 있어야 함.
    REQUIRED_FIELDS = []
...
class UserProfile(models.Model):
    user = models.OneToOneField(User, verbose_name="유저", on_delete=models.CASCADE) # 1 유저 1 프로필
